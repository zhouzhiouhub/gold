...
控制台「连接 GitHub」创建的是 Worker，部署命令是 `npx wrangler deploy`。静态页在 `public/`，接口在 `src/index.js` 的 `/api/gold`。页面分为国内现货、国际金、商家投资金价与金饰挂牌价。投资金价按现货加价估算；金饰价尽量抓取公开挂牌，抓取失败则显示 `--`。现货行情优先 GoldAPI，失败则走备用源。
...
src/gold-api.js      云上金价接口（GoldAPI / 备用源）
src/jewelry.js       品牌金饰挂牌抓取
